...
function effectiveConfigProps() {
    return configState.props.map(p =>
...
async function saveConfig() {
    ...
    const merged = effectiveConfigProps();
    try {
...
async function applyWorld() {
    ...
    // Merge into existing.
    const map = new Map(effectiveConfigProps().map(p => [p.name, p.value]));
...
async function deleteActiveWorld() {
    const world = $("#world-select").value || "(unset)";
    const game = $("#game-name").value.trim() || "(unset)";
    if (!confirm(`Delete the loaded save for:\n\n${world}/${game}\n\nThe server must be stopped. This permanently removes that save folder.`)) {
        return;
    }
    try {
        const r = await api.del("/api/worlds/active");
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    $("#btn-delete-active-world").addEventListener("click", deleteActiveWorld);
