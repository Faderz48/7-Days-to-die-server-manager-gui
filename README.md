...
| `GET`    | `/api/worlds`                 | list save folders                          |
| `GET`    | `/api/worlds/download/:name`  | download a save folder as zip              |
| `POST`   | `/api/worlds/upload`          | upload and extract a save zip              |
| `DELETE` | `/api/worlds/:name`           | delete a save folder                       |
| `DELETE` | `/api/worlds/active`          | delete the configured active save          |
| `GET`    | `/api/mods`                   | list installed modlets                     |
| `POST`   | `/api/mods/upload`            | upload and extract a modlet zip            |
| `DELETE` | `/api/mods/:name`             | delete an installed modlet                 |

---

## Caveats

- The manager binds `127.0.0.1:8421` by default. Use `BIND=0.0.0.0:8421`
  for LAN access, and put it behind something with auth if you expose it.
...
- World generation happens inside the running game, on first start of a
  ...
  directory.
- Release builds have been tested on Windows and Linux. The AppImage build
  currently requires `RUSTFLAGS='-A dead_code'` on Rust 1.95 because that
  compiler can ICE while rendering an otherwise harmless dead-code warning.
