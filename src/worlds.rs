...
/// Delete a save at a relative path under `saves_dir`, e.g.
/// `Navezgane/MySave`. Refuses absolute paths, `..`, and paths that
/// resolve outside the saves directory.
pub fn delete_save_relative(saves_dir: &Path, rel_path: &Path) -> Result<u64> {
    let safe = sanitize_relative_path(rel_path)
        .ok_or_else(|| anyhow!("invalid save path: {}", rel_path.display()))?;
    if safe.as_os_str().is_empty() {
        bail!("invalid save path: '{}'", rel_path.display());
    }

    let target = saves_dir.join(&safe);
    if !target.exists() {
        bail!("save folder not found: {}", target.display());
    }
    if !target.is_dir() {
        bail!("save path is not a folder: {}", target.display());
    }
