...
pub fn routes() -> Router<SharedState> {
    Router::new()
        ...
        .route("/api/worlds/active",         axum::routing::delete(delete_active_world))
...
async fn post_console_exec(
    ...
    let client = match state.read().await.telnet.clone() {
        Some(client) => client,
        None => {
            state.write().await.log_manager("telnet not attached; trying to connect now");
            match crate::telnet::connect(state.clone()).await {
                Ok(client) => {
                    state.write().await.telnet = Some(client.clone());
                    state.write().await.log_manager("telnet attached");
                    client
                }
                Err(e) => {
                    return Err(ApiError(
                        StatusCode::SERVICE_UNAVAILABLE,
                        format!("telnet not attached: {e:#}"),
                    ));
                }
            }
        }
    ...
    if let Err(e) = client.send(&cmd).await {
        ...
        state.write().await.log_manager(format!("telnet send failed: {e}; reconnecting"));
        let client = crate::telnet::connect(state.clone())
            .await
...
async fn delete_active_world(
...
) -> ApiResult<Json<DeleteResponse>> {
    {
        ...
        if s.status.is_alive() {
            return Err(ApiError(
                StatusCode::CONFLICT,
                "stop the server before deleting the active save".into(),
            ));
        }
    }

    let (saves_dir, world, game) = {
        let s = state.read().await;
        let saves = s.settings.saves_dir.clone()
            .ok_or_else(|| anyhow::anyhow!("saves directory not configured"))?;
        let cfg_path = s.settings.resolve_config_path()
            .ok_or_else(|| anyhow::anyhow!("server config path not set"))?;
        let cfg = ServerConfig::load(&cfg_path)?;
        let world = cfg.get("GameWorld")
            .map(str::trim)
            .filter(|v| !v.is_empty())
            .ok_or_else(|| ApiError(StatusCode::BAD_REQUEST, "GameWorld not set in serverconfig.xml".into()))?
            .to_string();
        let game = cfg.get("GameName")
            .map(str::trim)
            .filter(|v| !v.is_empty())
            .ok_or_else(|| ApiError(StatusCode::BAD_REQUEST, "GameName not set in serverconfig.xml".into()))?
            .to_string();
        (saves, world, game)
    };

    let nested_rel = PathBuf::from(&world).join(&game);
    let direct_rel = PathBuf::from(&game);
    let rel = if saves_dir.join(&nested_rel).exists() {
        nested_rel
    } else if saves_dir.join(&direct_rel).exists() {
        direct_rel
    } else {
        nested_rel
    };
    let bytes = tokio::task::spawn_blocking({
        let saves_dir = saves_dir.clone();
        let rel = rel.clone();
        move || crate::worlds::delete_save_relative(&saves_dir, &rel)
    })
    .await
    ...
    state.write().await.log_manager(format!(
        "active save '{name}' deleted ({} bytes freed)", bytes,
    ));
